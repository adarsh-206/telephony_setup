...
const AUDIO_FILES = ['./sounds/voice_1.alaw', './sounds/voice_2.alaw', './sounds/voice_3.alaw'];
    ...
    ws.on('message', (msg) => {
        try {
            ...
            if (message.type === 'start') {
                ...
                // 1. Send initial burst to fill the Jitter Buffer
                for (let i = 0; i < 3; i++) {
                ...
                // 2. Set the high-precision timeline AFTER the burst
                ...
                const stream = () => {
                    ...
                    // 3. Increment the objective timeline
                    nextTick += intervalNS;

                    const now = process.hrtime.bigint();
                    const delayMS = Number(nextTick - now) / 1_000_000;

                    // 4. Schedule next chunk based on the calculated delay
                    audioTimeout = setTimeout(stream, Math.max(0, delayMS));
                };

                // Start the rhythmic loop
                stream();
            }
        } catch (e) { }
    });

    ws.on('close', () => { if (audioTimeout) clearTimeout(audioTimeout); });
    ws.on('error', () => { if (audioTimeout) clearTimeout(audioTimeout); });
});
